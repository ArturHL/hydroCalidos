...
- Sin autenticación real: hay un **selector de rol** manual en el header (`Checador salida` / `Checador destino` / `Contador (Constructora)` / `Contador (Transportista)` / `Dueño (Constructora)` / `Dueño (Transportista)` / `RH`) que decide qué navegación y acciones ve cada quien.
...
src/
  ...
  components/
    PhotoCaptureField.jsx  Cámara propia (getUserMedia, cámara trasera forzada) para la evidencia del camión cargado
  pages/
...
1. **Formulario de salida** (`/formulario-salida`, rol Checador salida): paso 1 del viaje — Origen (banco) **autocompletado con el "lugar de trabajo" registrado en el perfil de RH del checador** (`checadorPorNombre()`, `PersonalContext`) — sin GPS de este lado, porque el banco ya está fijo en su perfil (no como el cadenamiento del destino, que sí varía). Si el checador lo cambia a un banco distinto al asignado, exige una justificación de texto (`justificacionOrigen`) para poder enviar — mismo patrón anti-fraude que la distancia en el paso 2. → filtra Material disponible. Placa es un catálogo cerrado sacado de los Operadores que RH dio de alta — elegirla **autocompleta Nombre del operador (solo lectura, va ligado 1:1 al camión), Capacidad nominal y Representante del Transportista** (estos dos últimos editables). Volumen real cargado, Checador (readOnly, `CHECADOR_SALIDA_ACTUAL` fijo en `FormSalidaPage.jsx`, simula sesión iniciada) y Coordenadas de salida — **autocompletadas con las coordenadas fijas del banco** (`BANCO_COORDS`/`coordenadasDeBanco()` en `mockContract.js`, se actualizan si el banco cambia), editables, sin pedir GPS de este lado tampoco. **Evidencia fotográfica obligatoria**: 2 fotos del camión ya cargado (frente y atrás), tomadas con `PhotoCaptureField.jsx` — cámara propia dentro de la página (`getUserMedia`, `facingMode: { exact: 'environment' }`), nunca la app nativa de cámara del celular ni el carrete/galería del dispositivo; si el dispositivo no tiene cámara trasera, falla con un error claro en vez de usar la frontal en silencio (feedback del socio, 2026-08-28). Sin costo — todavía no hay distancia. **Mejoras anti-error (2026-08-28, ver sección dedicada más abajo):** select de Placa con buscador de texto + orden alfabético, tarjeta de confirmación destacada del camión (Operador/Capacidad/Representante), aviso si el Volumen se aleja mucho de la Capacidad, y pantalla de revisión ("Confirmar y enviar" / "Seguir editando") antes de registrar la salida. Al confirmar, `addSalida()` (`TripsContext`) crea el viaje con `estado: 'en_transito'` y navega a `/salida/:id` (confirmación simple, no es el Ticket).
2. **Formulario de llegada** (`/formulario`, rol Checador destino): paso 2 — selecciona de una lista el viaje **en tránsito** que llegó (folio + placa + origen), ve en solo lectura lo que el Checador de salida ya capturó (origen, material, placa, operador, volumen, Checador de salida), y completa Destino (cadenamiento). Botón **"Detectar cadenamiento por GPS"** (`navigator.geolocation` + `cadenamientoMasCercano()` en `mockContract.js`, coincidencia por cercanía tipo Haversine contra los 6 puntos conocidos del tramo, `src/utils/geo.js`) — autocompleta el destino si hay un punto conocido a menos de 400 m; si no, pide selección manual (sin match, permiso denegado, o sin soporte de geolocalización, cada caso con su propio mensaje). Distancia se autosugiere según la ruta; si el checador cambia la distancia **o** el cadenamiento detectado por GPS, exige justificación (`justificacion`, un solo campo, dispara con cualquiera de los dos motivos). **También 2 fotos del camión cargado (frente y atrás)**, mismo componente `PhotoCaptureField.jsx` que el Checador salida — confirma que llegó con la carga completa, antes de descargar. Checador (readOnly, `CHECADOR_DESTINO_ACTUAL` fijo) y Coordenadas de llegada — **también se autocompletan con la misma lectura de GPS** que usó el botón de arriba (la lectura cruda, no el punto del catálogo; se guarda haya o no coincidencia de cadenamiento), editables. Si no hay viajes en tránsito, empty-state pidiendo esperar al Checador de salida. El costo estimado **no se muestra en ningún punto de este flujo** (ni salida ni destino ven montos — se calcula y guarda para Registros/Métricas/Conciliación, pantallas de Contador). **Mejoras anti-error (2026-08-28):** lista de viajes en tránsito ordenada del más antiguo al más reciente, fotos de salida visibles junto al camión real para comparar antes de completar, y pantalla de revisión antes de enviar (ver sección dedicada). Al confirmar, `completarLlegada()` marca `estado: 'completado'` y navega a `/ticket/:id` — aquí es donde se genera el Ticket, no en el paso 1.
3. **Ticket generado** (`/ticket/:id`): resumen de datos (sin costo, incluye Checador de salida y Checador de destino) + **grid de las 4 fotos del camión cargado** (2 de salida + 2 de llegada, solo las que existan) + botón Imprimir (`window.print()`, básico, sin diseño de impresión elaborado) + volver al Formulario de llegada.
4. **Registros** (`/registros`, rol Contador): tabla de todos los viajes (en tránsito y completados), con columna "Estado" (badge "En tránsito"/"Completado"), columnas separadas "Checador salida"/"Checador destino", y badge de "Excepción" cuando la distancia no coincidió con la esperada. Las fotos del camión cargado (como las coordenadas) no tienen columna aquí ni en Conciliación — solo se ven en el Ticket (`/ticket/:id`) por ahora; sigue siendo el dato completo del viaje, disponible para cuando haga falta mostrarlo en otro lado.
...
6. **Contrato actual** (`/contrato`, roles Dueño y Contador — alcance movido el 2026-08-28, ver abajo): mismo patrón de negociación que Conciliación pero sobre las tarifas (5 bandas de distancia × 3 categorías de material — ver más abajo), **incluyendo los mismos dos arreglos de feedback**: "Modificar y reenviar" (segunda ronda en adelante) tiene botón "Cancelar", y "Aceptar revisión" pide confirmación en pantalla antes de aplicarse (la primera ronda, "Solicitar revisión", ya tenía su propio "Cancelar" desde antes). **Solo el Dueño (Constructora/Transportista) puede solicitar/negociar/aceptar una revisión** (`puedeEditar` en `ContratoPage.jsx`) — el Contador entra a la misma pantalla pero en solo lectura: ve la tabla de tarifas vigentes y, si hay una negociación en curso, la ve avanzar (con quién es el turno, las rondas) sin ningún botón de acción, y un aviso explicando por qué. Coincide con lo ya confirmado para el sistema real (`Volteo/docs/REQUIREMENTS.md` RF-1.4: "Dueño/Representante... Único que puede crear y modificar el contrato") — la maqueta tenía esto invertido hasta ahora. Tab "Acuerdo actual" (tabla vigente + Solicitar revisión, si aplica) y tab "Historial" (visible para ambos roles). También lista el catálogo de bancos/materiales como referencia de solo lectura.
...
8. **Panel del Dueño** (`/panel-dueno`, roles Dueño (Constructora)/Dueño (Transportista) — dividido por lado el 2026-08-28, mismo patrón que ya existía para Contador; antes era un solo rol genérico `dueno`, migración automática de ese valor viejo a `dueno_constructora` en `RoleContext.jsx`): KPIs de solo lectura (viajes totales, costo acumulado, excepciones abiertas, conciliaciones cerradas — solo viajes **completados**) + historial de conciliaciones. Cero botones de edición/negociación en esta pantalla — el rol confirmado por el socio ("Duenos y representantes... sin acceso a modificar datos, solo verlos", `docs/business/CUESTIONARIO_SOCIO.md` §0 en Volteo) sigue aplicando aquí. La excepción es Contrato (ítem 6, arriba): ahí el Dueño sí edita — es la única pantalla donde tiene permiso de escritura, coincidiendo con RF-1.4 del sistema real. Verificado con Puppeteer headless que no aparece ningún botón de guardar/enviar/proponer/aceptar/descargar en el Panel mismo.
9. **RH** (`/rh`, rol RH): alta de Checadores (nombre + **tipo: salida o destino** + lugar de trabajo/obra) y Operadores (nombre + placa + Representante del Transportista asignado + capacidad del camión), persistidos en `PersonalContext`/`localStorage` (`volteo_checadores`, `volteo_operadores`). El campo "lugar de trabajo" **depende del tipo**: para Checador salida es un select cerrado de Bancos (`BANCOS` de `mockContract.js`) — es la llave que usa el Formulario de salida para autocompletar el Origen; para Checador destino sigue siendo texto libre (su cadenamiento se detecta por GPS en el Formulario de llegada, no depende de este campo). Reemplaza los catálogos estáticos `mockCamiones.js`/`mockOperadores.js` (eliminados) — ahora Placa/Operador en los Formularios salen de perfiles reales dados de alta aquí, no de un mock hardcodeado. RH no puede dar de alta dos Operadores con la misma placa (`placaEnUso()`, error en pantalla si se intenta). Tabs "Checadores"/"Operadores", cada uno con su formulario de alta + tabla con botón de eliminar por fila. **Tab "Estadísticas" (2026-08-28):** KPIs de solo lectura (Checadores/Operadores activos, viajes y viajes-con-excepción de los últimos 30 días — ventana móvil, no mes calendario) + dos tablas ordenables (`records-toolbar` reutilizado de Registros): Checadores (Viajes / Tickets expedidos — solo aplica a tipo destino, "—" en salida / Excepciones / Productividad = viajes ÷ días distintos con actividad) y Operadores (Viajes / Volumen transportado / Viajes con excepción / Productividad). "Errores" se calcula reutilizando datos que el sistema ya captura, sin inventar un concepto nuevo: `justificacionOrigen` (banco distinto al asignado) para Checador salida, `excepcion` (distancia/cadenamiento) para Checador destino y para Operador. Simplificación conocida: la ventana de 30 días se mide sobre `timestamp` (fecha de salida, nunca se actualiza al completarse) — para Checador destino esto asume que la llegada ocurre el mismo día que la salida, cierto en el flujo real pero no garantizado por el modelo de datos.

## Mejoras anti-error de captura del Checador (2026-08-28)

El objetivo explícito del socio: no se puede eliminar el error humano del todo, pero sí seguir reduciendo la probabilidad de que ocurra en la captura (que es inmutable — el Checador no puede corregirlo después). Seis mejoras, todas en `FormSalidaPage.jsx`/`FormPage.jsx`:

1. **Pantalla de revisión antes de enviar, en ambos Formularios.** El botón principal ya no envía directo — valida y pasa a un resumen (`revisando`, estado local) con todos los datos capturados **incluidas las miniaturas de fotos**, más "Confirmar y enviar" / "Seguir editando" (regresa al formulario sin perder nada de lo ya llenado). Mismo patrón que ya se usaba en Conciliación/Contrato, aplicado aquí por primera vez — es, de hecho, el punto de mayor riesgo de todo el sistema (más volumen, y la única captura sin ninguna posibilidad de corrección directa).
2. **Fotos de salida visibles para el Checador destino al elegir el viaje.** Antes de completar un viaje en tránsito, ve las 2 fotos que ya tomó el Checador de salida junto a los datos de texto — comparación visual directa contra el camión que tiene enfrente, antes de seguir. Mitiga el riesgo real de completar el viaje equivocado cuando hay varios camiones en tránsito a la vez. Se repite en la pantalla de revisión final (con las 4 fotos, 2 de salida + 2 de llegada).
3. **Aviso suave sobre Volumen fuera de rango.** Si el volumen capturado se aleja mucho de la capacidad nominal del camión (`> capacidad × 1.15` o `< capacidad × 0.3`), aparece un aviso (`.field-warning`, no bloquea por sí solo) y una casilla "Confirmo que el volumen es correcto" que sí hay que marcar para pasar a revisión — atrapa el típico error de dedo (150 en vez de 15) sin exigir una justificación completa como las excepciones de negocio reales.
4. **Sin doble envío.** El botón "Confirmar y enviar" se deshabilita mientras procesa (`enviando`, estado local) — evita folios duplicados por doble tap.
5. **Selects que van a crecer, ordenados y con buscador.** El select de Placa (Formulario de salida) ahora está ordenado alfabéticamente y tiene un campo de texto que filtra las opciones en vivo — hoy son 10 operadores, pero un select plano sin buscar se vuelve su propio riesgo con una flota más grande. La lista de "viajes en tránsito" (Formulario de llegada) quedó ordenada del más antiguo al más reciente en vez de sin orden — sin buscador ahí, ese listado no debería crecer tanto como para necesitarlo.
6. **Tarjeta de confirmación destacada del camión.** Al elegir la Placa, aparece un `.confirm-card` (Operador, Capacidad, Representante) en vez de que ese dato viva solo como un input de solo lectura chico entre muchos otros campos — más difícil de no notar si algo autocompletó mal.

No se construyó (evaluado y descartado para esta ronda): formulario por pasos/wizard en vez de una sola pantalla larga — mayor alcance de rediseño de UX, se deja como palanca a considerar después si el error sigue siendo un problema con lo demás ya en producción.

Los flujos de Conciliación y Contrato actual, y el flujo Formulario→Ticket→Registros→Métricas, se probaron end-to-end con scripts de Puppeteer headless (no versionados, se usaron y se borraron en la sesión) — pasaron todos los checks, incluyendo que el costo calculado coincide exacto con la fórmula esperada. El flujo de RH (alta de Checador/Operador, rechazo de placa duplicada, autocompletado de Nombre/Capacidad/Representante al elegir placa en el Formulario, y que eliminar un Operador desde RH lo quita del select de Placa) también se probó end-to-end con Puppeteer (18/18 checks). El flujo de dos Checadores (salida abre con `addSalida()`, destino lo completa con `completarLlegada()` y genera el Ticket, el viaje desaparece de "en tránsito" una vez cerrado, Conciliación/Métricas/Panel del Dueño excluyen los viajes en tránsito) también se probó end-to-end con Puppeteer (27/27 checks). El autocompletado por perfil (banco) y por GPS (cadenamiento) — incluyendo el `overridePermissions`/`page.setGeolocation()` de Puppeteer para simular ubicación, la coincidencia correcta contra un punto conocido, el caso "sin match" con una ubicación lejana (CDMX), y que cambiar cualquiera de los dos autocompletados exige justificación — también se probó end-to-end con Puppeteer (12/12 checks). El autocompletado de las coordenadas mismas (Coordenadas de salida desde el catálogo del banco, Coordenadas de llegada desde la lectura de GPS incluso sin match de cadenamiento) también se probó end-to-end con Puppeteer (5/5 checks). Los botones de "Cancelar" (contraoferta) y confirmación (aceptar) en Conciliación y Contrato — que cancelar de verdad descarta los cambios y no envía nada, que confirmar de verdad cierra la negociación y agrega la entrada al Historial — también se probaron end-to-end con Puppeteer (13/13 checks). La cámara de evidencia (`PhotoCaptureField.jsx`) se probó end-to-end con Puppeteer usando `--use-fake-device-for-media-stream` — en una página sin parchar (simula un dispositivo sin cámara trasera real, como esta laptop de desarrollo) confirma que `facingMode: exact: 'environment'` falla con un error claro en vez de usar la frontal, que "Reintentar" y cerrar el modal funcionan, y que enviar sin las 2 fotos bloquea con su propio mensaje; en una página con `getUserMedia` parchado para ignorar el constraint exacto (simula un teléfono real con cámara trasera) se probó el flujo completo — vista previa en vivo, capturar, revisar, usar/volver a tomar, miniatura, envío exitoso en ambos Formularios, y que el Ticket muestra las 4 fotos — 14/14 checks. Una foto capturada por el dispositivo de prueba de Chrome pesó ~8 KB como data URL (recorte a 900px + JPEG calidad 0.65 en `PhotoCaptureField.jsx`); una foto real de celular pesará más pero debería mantenerse muy por debajo de la cuota de `localStorage` (~5-10 MB) salvo que se acumulen muchísimos viajes con fotos sin limpiar. Las 6 mejoras anti-error también se probaron end-to-end con Puppeteer (15/15 checks): orden alfabético y filtro de texto del select de Placa, la tarjeta de confirmación del camión, el aviso de volumen fuera de rango bloqueando el paso a revisión hasta confirmarlo, que "Seguir editando" conserva los datos ya capturados, que la revisión de llegada muestra las 4 fotos (2 de salida + 2 de llegada), y que ambos "Confirmar y enviar" sí completan el registro real. El movimiento de la edición de Contrato de Contador a Dueño también se probó end-to-end con Puppeteer (16/16 checks): el selector de rol trae las dos opciones nuevas de Dueño (y ya no la genérica), Dueño Constructora puede solicitar una revisión y Dueño Transportista aceptarla, ambos Contadores ven la negociación avanzar sin ningún botón de acción disponible ni inputs editables, que sin negociación activa el Contador no ve "Solicitar revisión" (solo el aviso de solo lectura + la tabla vigente), y que el Historial resultante queda atribuido a los Dueños. La tab "Estadísticas" de RH también se probó end-to-end con Puppeteer (13/13 checks): los 4 KPIs superiores coinciden exacto con lo esperable de la semilla (7 Checadores, 10 Operadores, 23 viajes en 30 días, 1 con excepción), la fila de un Checador destino muestra sus 13 viajes + 1 excepción + badge de tickets expedidos, la de un Checador salida no muestra tickets (—), el orden por defecto (viajes, mayor a menor) y reordenar por nombre (alfabético) funcionan, y ambas tablas (Checadores/Operadores) están presentes.
...
- **En tránsito** (folio 23, hoy, origen Banco Las Torres): un viaje que el
  ...
  "sin match" y se elige a mano, ambos casos están cubiertos). Trae 2 fotos
  placeholder (`fotoPlaceholder()`, SVG en línea, sin depender de un
  archivo binario) para que el Checador destino tenga algo que comparar en
  vivo — los 22 viajes ya completados de la semilla no llevan fotos (son
  historial, no necesitan demostrar el flujo de captura).
